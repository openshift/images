...
func exactNodeDetails(oc *exutil.CLI) (int, string) {
	// Use JSON output to count ready worker nodes
	nodesJSON, err := oc.AsAdmin().WithoutNamespace().Run("get").Args("nodes", "--selector=node-role.kubernetes.io/worker=,kubernetes.io/os=linux", "-o", "json").Output()
	...
	for _, node := range nodes.Items {
		for _, cond := range node.Status.Conditions {
			if cond.Type == "Ready" && cond.Status == "True" {
				readyNodeNames = append(readyNodeNames, node.Metadata.Name)
				break
			}
		}
	}
	nodeCount := len(readyNodeNames)
	e2e.Logf("Linux worker node details:\n%v", strings.Join(readyNodeNames, "\n"))
	...
	return nodeCount, strings.Join(readyNodeNames, "\n")
}
...
func ensureIpfailoverMasterBackup(oc *exutil.CLI, ns string, podList []string) (string, string) {
	o.Expect(len(podList)).To(o.BeNumerically(">=", 2), "ensureIpfailoverMasterBackup requires at least 2 pods, got %d", len(podList))
...
func getPodListByLabel(oc *exutil.CLI, namespace string, label string) []string {
	...
	o.Expect(err).NotTo(o.HaveOccurred())
	podList = strings.Fields(podNameAll)
	o.Expect(podList).NotTo(o.BeEmpty(), "no pods found with label %s in namespace %s", label, namespace)
...
func getVipOwnerPod(oc *exutil.CLI, ns string, podname []string, vip string) string {
	...
	for i := 0; i < len(podname); i++ {
		...
		// Check err first - exit code 1 from grep means VIP not found
		if err != nil {
	...
	o.Expect(primaryNode).NotTo(o.BeEmpty(), "no pod found owning VIP %s", vip)
	return primaryNode
}
...
func waitForPrimaryPod(oc *exutil.CLI, ns string, pod string, vip string) {
	...
	waitErr := wait.Poll(5*time.Second, 50*time.Second, func() (bool, error) {
		...
		// Check err first - exit code 1 from grep means VIP not assigned yet
		if err != nil {
			if strings.Contains(err.Error(), "exit status 1") {
				e2e.Logf("pod failed to become master yet, retrying...")
				return false, nil
			}
			// Any other error is a real failure
			return false, err
...
func getPodIP(oc *exutil.CLI, namespace string, podName string) []string {
	...
	o.Expect(podIp).NotTo(o.BeEmpty(), "could not determine pod IP for %s in namespace %s", podName, namespace)
	return podIp
}
...
func getSpecificPodLogs(oc *exutil.CLI, namespace string, container string, podName string, filter string) (string, error) {
	...
	data, errRead := os.ReadFile(podLogs)
	if errRead != nil {
		e2e.Logf("unable to read the pod logs file: %s", podLogs)
		return "", errRead
	}
	if len(filter) == 0 {
